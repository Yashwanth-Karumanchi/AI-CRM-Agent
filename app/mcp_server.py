...
async def check_health() -> dict:
    """Check whether the ARIA CRM agent is running"""
    return {
        "ok":      True,
...
async def create_client(data: CreateClientInput) -> dict:
    ...
    Validates email uniqueness, logs activity.
    """
    try:
...
async def get_pipeline_summary() -> dict:
    """Get pipeline stats, stage counts, high priority clients"""
    try:
...
@mcp.tool()
async def generate_client_report_tool(
    ...
    try:
        ...
        asyncio.create_task(sheets.log_activity(
            data.client_id, "REPORT_GENERATED",
            f"Word report for {client['name']}",
            "SUCCESS", filename
        ))
...
async def ai_draft_email(data: EmailDraftInput) -> dict:
    """
    AI drafts a professional email for a client.
    Optionally send immediately or save as Gmail draft.
    """
    try:
        ...
        if data.send:
            ...
            asyncio.create_task(sheets.log_activity(
                ...
                f"Sent to {recipient}: {drafted['subject']}",
                "SUCCESS", ""
            ))
            return {
                "ok":       True,
            ...
            asyncio.create_task(sheets.log_activity(
                data.client_id, "EMAIL_DRAFT",
                f"Draft for {recipient}: {drafted['subject']}",
...
@mcp.tool()
async def send_email_tool(data: SendEmailInput) -> dict:
    ...
    try:
        ...
        await sheets.require_client(data.client_id)
        ...
        asyncio.create_task(sheets.log_activity(
            data.client_id, "EMAIL_SENT",
            f"Sent to {data.to}: {data.subject}",
            "SUCCESS", ""
        ))
        return {
            "ok":      True,
